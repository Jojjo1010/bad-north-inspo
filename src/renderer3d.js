...
export class Renderer3D {
  ...
  drawLevelUpMenu(level, powerups, hoveredIndex, train) {
    ...
    // ========== CREW & WEAPON STATUS PANEL (bottom) ==========
    if (train) {
      ...
      const slotW = 180;
      const slotGap = 20;
      const crewCount = train.crew.length;
      // Garlic weapon slot
      const hasGarlic = train.hasAutoWeapon('steamBlast');
      const totalSlots = crewCount + (hasGarlic ? 1 : 0);
      ...
      for (let i = 0; i < crewCount; i++) {
        ...
        const sy = panelY + 8;
        const roleEmoji = crewEmoji(c);
        const roleColor = c.role === 'Gunner' ? '#ffb74d' : c.role === 'Brawler' ? '#66bb6a' : '#888';
        const gunStats = MANUAL_GUN.levels[Math.max(0, c.gunLevel - 1)];

        // Crew color accent bar
        ctx.fillStyle = c.color;
        ctx.fillRect(sx, sy, 3, 52);

        // Avatar
        ctx.font = '22px serif';
        ctx.textAlign = 'left';
        ctx.fillText(roleEmoji, sx + 10, sy + 22);

        // Name + role
        ctx.fillStyle = '#fff';
        ctx.font = 'bold 11px monospace';
        ctx.fillText(c.name || `Crew ${i + 1}`, sx + 38, sy + 14);
        ctx.fillStyle = roleColor;
        ctx.font = '9px monospace';
        ctx.fillText(c.role || 'No role', sx + 38, sy + 26);

        // Gun stats
        ctx.fillStyle = '#999';
        ctx.font = '10px monospace';
        ctx.fillText(`Gun Lv${c.gunLevel}/${MANUAL_GUN.maxLevel}`, sx + 38, sy + 40);
        ctx.fillStyle = '#777';
        ctx.font = '9px monospace';
        ctx.fillText(`DMG ${gunStats.damage} | Rate ${gunStats.fireRate.toFixed(1)}/s`, sx + 38, sy + 52);
      }

      // Garlic slot
      if (hasGarlic) {
        const gx = slotsStartX + crewCount * (slotW + slotGap);
        const gy = panelY + 8;
        const gStats = train.getAutoWeaponStats('steamBlast');
        const gMount = train.getAutoWeaponMount('steamBlast');
        const gDisabled = gMount && gMount._bandit;

        // Accent bar
        ctx.fillStyle = gDisabled ? '#a33' : '#66bb6a';
        ctx.fillRect(gx, gy, 3, 52);

        // Icon
        ctx.font = '20px serif';
        ctx.textAlign = 'left';
        ctx.fillText('\uD83D\uDCA8', gx + 10, gy + 22);

        // Name
        ctx.fillStyle = gDisabled ? '#a55' : '#8ecae6';
        ctx.font = 'bold 11px monospace';
        ctx.fillText('Garlic Aura', gx + 38, gy + 14);
        ctx.fillStyle = gDisabled ? '#a33' : '#666';
        ctx.font = '9px monospace';
        ctx.fillText(gDisabled ? 'DISABLED' : 'Active', gx + 38, gy + 26);

        // Stats
        ctx.fillStyle = '#777';
        ctx.font = '9px monospace';
        if (gStats) {
          ctx.fillText(`DMG ${gStats.damage}/tick | Radius ${gStats.radius}`, gx + 38, gy + 40);
          ctx.fillText(`Lv${train.autoWeapons.steamBlast.level}`, gx + 38, gy + 52);
        }
      }
    }
  }
